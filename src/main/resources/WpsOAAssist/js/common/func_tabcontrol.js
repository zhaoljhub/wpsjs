...
function OnInsertRedHeaderClick() {
    ...
    var l_insertFileUrl = GetDocParamsValue(l_Doc, constStrEnum.customRedFileListUrl); //插入文件的位置
    //var l_BkFile = GetDocParamsValue(l_Doc, constStrEnum.bkInsertFile);
    if (l_insertFileUrl != "") {
        var height = 400;
        var width = 400;
        OnShowDialog("customRedhead.html", "OA助手", height, width);
        return;
    } else {
        alert("参数customRedFileListUrl为空！");
        return;
    }
}
...
function OnUploadToServerSuccess(resp, fileExt) {
    ...
    // todo-2 添加事件回调机制，通知页面做了什么操作
    var info = {serverResponse: resp, wpsInfo: wps.PluginStorage.getItem(l_doc.DocID)}
