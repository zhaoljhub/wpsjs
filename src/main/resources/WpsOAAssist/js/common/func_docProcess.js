...
function pOpenFile(doc, params, isOnlineDoc) {
    ...
    var ReplaceDoc = GetParamsValue(params, constStrEnum.methodType)
    if (ReplaceDoc && ReplaceDoc == "ReplaceDoc") {
        customInsertRepFile(doc);
    }
    ...
    // 套红头
    if (GetParamsValue(params, constStrEnum.customRedFileUrl)) {
        customWpsRedFile(doc);
    }

    // 替换模板
    /*if(GetParamsValue(params, constStrEnum.repFileElement)){
        customWpsRepFile(doc);
    }*/
        ...
        if (l_Doc) {
            if (pCheckIfOADoc(l_Doc) == true) { // 是否为OA文件
                if (pISOADocReadOnly(l_Doc) == false) { // 是否为只读文档
                    //todo-7 执行自动上传到OA服务器端的操作 , 该文件符合自动上传并大于3分钟
                    var value = GetDocParamsValue(l_Doc, constStrEnum.autoSaveToServer)
                    if (value) {
